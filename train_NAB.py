...
from models_2 import _netD, _netG, _param
...
def cal_triplets_loss(anchor, train_dic, margin):
    _len = anchor.shape[0]
    neg_loss = Variable(torch.Tensor([0.0])).cuda()
    ...
    for i in range(_len):
        ...
        if len((train_dic[i])) == 0:
            emptySetNum += 1
            continue;
        positive = Variable(torch.from_numpy(np.array(
            random.sample(train_dic[i], pos_num if pos_num <= len((train_dic[i])) else len((train_dic[i])))))).cuda()
        ...
        negative_loss = torch.div(negative_loss, Variable(torch.Tensor([len(other_cls) - otherEmptySetN])).cuda())
        neg_loss = torch.add(neg_loss, negative_loss)

    neg_loss = torch.div(neg_loss, Variable(torch.Tensor([_len - emptySetNum])).cuda())
    pos_loss = torch.div(pos_loss, Variable(torch.Tensor([_len - emptySetNum])).cuda())
    ...
    while True:
        if not first:
            ...
            for it in range(start_step, 4000 + 1):
                ...
                if it % opt.disp_interval == 0 and it:
                    acc_real = (np.argmax(C_real.data.cpu().numpy(),
                                          axis=1) == y_true.data.cpu().numpy()).sum() / float(y_true.data.size()[0])
                    acc_fake = (np.argmax(C_fake.data.cpu().numpy(),
                                          axis=1) == y_true.data.cpu().numpy()).sum() / float(y_true.data.size()[0])
        ...
        for i, label in enumerate(selectedHighConvinceIndex_y):
            ...
            if label in unsupervisedData.unsupervised_label_mapping:
                ...
                dataset.pfc_feat_data_train = np.insert(dataset.pfc_feat_data_train, insertIndex,
                                                        values=unsupervisedData.image_feature[
                                                            selectedHighConvinceIndex[i]], axis=0)
