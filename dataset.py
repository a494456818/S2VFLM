...
class LoadDataset(object):
    def __init__(self, opt):
        txt_feat_path = opt.txt_feat_path
        ...
        self.tr_cls_centroid = np.zeros([train_cls_num, self.pfc_feat_data_train.shape[1]]).astype(np.float32) # 每个类的类中心
        for i in range(train_cls_num):
            self.tr_cls_centroid[i] = np.mean(self.pfc_feat_data_train[self.labels_train == i], axis=0) # 图片的类中心
